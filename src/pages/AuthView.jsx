...
export default function AuthView() {
  ...
  async function handleLogin(e) {
    ...
    if (res.success) {
      // Strictly enforce that admins can ONLY use the hidden portal
      if (res.user.role.toLowerCase() === 'admin') {
        await logout();
        setError('Administrators must use the secure hidden admin portal (click logo 10 times).');
      } else {
  ...
  async function handleRegister(e) {
    ...
    if (activeRole === 'student') {
      setPendingRegData({ email, password, name, activeRole, extraData });
      setShowScanner(true);
      return;
    }

    await completeRegistration(email, password, name, activeRole, extraData);
  }

  async function completeRegistration(email, password, name, activeRole, extraData) {
  ...
  async function handleFaceCapture(descriptorStr) {
    setShowScanner(false);
    
    // Handle Student Registration Face Verification
    const { email, password, name, activeRole, extraData } = pendingRegData;
    completeRegistration(
      email, 
      password, 
      name, 
      activeRole, 
      { ...extraData, face_id: descriptorStr }
    );
  }
  ...
  return (
    <div className="auth-container">
      <section className="auth-banner">
        <div className="brand-logo" onClick={handleLogoClick} style={{ cursor: 'pointer', userSelect: 'none' }}>
      ...
      <section className="auth-form-shell">
        <div className="auth-card">
          <div className="auth-tabs" style={{ display: mode === 'admin_login' ? 'none' : 'flex' }}>
            <button
          ...
          {mode === 'admin_login' && (
            <div style={{ marginBottom: '20px', textAlign: 'center' }}>
              ...
              <h2 style={{ fontSize: '18px', color: 'var(--text-main)', marginBottom: '4px' }}>Secure Admin Portal</h2>
              <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Requires Master Password</p>
            </div>
          )}
          ...
          {mode === 'login' ? (
            ...
            <form onSubmit={handleAdminSubmit}>
              <div className="form-group">
                ...
                <input name="adminEmail" type="email" className="input-field" placeholder="admin@attendly.com" required />
              </div>
              <div className="form-group">
                <label>Master Password</label>
                <div style={{ position: 'relative' }}>
                  <input name="adminPassword" type={showPassword ? "text" : "password"} className="input-field" placeholder="Enter master password" required style={{ paddingRight: '40px' }} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: 'var(--text-muted)' }}>
                    {showPassword ? 'HIDE' : 'SHOW'}
                  </button>
                </div>
              </div>
              <button type="submit" className="btn btn-emerald" style={{ width: '100%', marginTop: '16px' }} disabled={submitting}>
                {submitting ? 'Verifying...' : 'Access Admin Portal'}
              </button>
              <button type="button" className="btn btn-secondary" style={{ width: '100%', marginTop: '8px' }} onClick={() => setMode('login')}>
            ...
            <form onSubmit={handleRegister}>
              ...
              <button type="submit" className="btn btn-emerald" style={{ width: '100%', marginTop: '8px' }} disabled={submitting}>
                {submitting ? 'Creating Account...' : (activeRole === 'student' ? 'Scan Face & Register' : 'Create Real Account')}
              </button>
            </form>
          )}
        </div>
